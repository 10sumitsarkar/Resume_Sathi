...
export default function ResumeTemplate3({ additionalClass, isStatic = false, resumeId, isForDownload = false }) {
    ...
    const Header = ({ name, jobTitle, city, state, phone, email, website, photo, isStatic: s }) => (
                ...
                <ul className="rt3_contact_list">
                    {(city || state) && (
                        <li>
                            ...
                            <span className="resume-address">{[city, state].filter(Boolean).join(', ')}</span>
                        </li>
                    )}
                    {phone && (
                        <li>
                        ...
                        <div className="rt3_body">
                            ...
                            <div className="rt3_main">
                                ...
                                <div className="rt3_block">
                                    ...
                                    {[
                                        { title: 'Senior UI/UX Designer', company: 'Pixel Labs', date: '2019 – Present', sub: 'Remote', desc: 'Led a team of designers for a fintech product, improving user retention by 30%.' },
                                        { title: 'UX Designer', company: 'CreativeSoft', date: '2016 – 2019', sub: 'Austin, TX', desc: 'Designed mobile apps and web dashboards for health and e-commerce clients.' },
                                    ].map((e, i) => (
                                        <div className="rt3_entry" key={i}>
                                            <div className="rt3_entry_head">
                                                <p className="rt3_entry_title">{e.title} — {e.company}</p>
                                                <p className="rt3_entry_date">{e.date}</p>
                                            </div>
                                            <p className="rt3_entry_sub">{e.sub}</p>
                                            <p className="rt3_entry_desc">{e.desc}</p>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="rt3_block">
                                    ...
                                    {[
                                        { title: 'B.A. in Graphic Design', sub: 'University of Texas, Austin', date: '2012 – 2016' },
                                        { title: 'Certified UX Specialist', sub: 'Nielsen Norman Group', date: '2018' },
                                    ].map((e, i) => (
                                        <div className="rt3_entry" key={i}>
                                            <div className="rt3_entry_head">
                                                <p className="rt3_entry_title">{e.title}</p>
                                                <p className="rt3_entry_date">{e.date}</p>
                                            </div>
                                            <p className="rt3_entry_sub">{e.sub}</p>
                                        </div>
                                ...
                                {work_experiences?.length > 0 && (
                                    <div className="rt3_block">
                                        ...
                                        {work_experiences.map((work, i) => (
                                            <div className="rt3_entry" key={i}>
                                                <div className="rt3_entry_head">
                                                    <p className="rt3_entry_title">{work.job_title} — {work.company_name}</p>
                                                    <p className="rt3_entry_date">
                                                        {work.start_month} {work.start_year} –{' '}
                                                        {(!work.end_month || !work.end_year) ? 'Present' : `${work.end_month} ${work.end_year}`}
                                                    </p>
                                                </div>
                                                <p className="rt3_entry_sub">{work.location}{work.employee_type ? ` • ${work.employee_type}` : ''}</p>
                                                {work.description && <p className="rt3_entry_desc">{work.description}</p>}
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {any_internships?.length > 0 && (
                                    <div className="rt3_block">
                                        ...
                                        {any_internships.map((intern, i) => (
                                            <div className="rt3_entry" key={i}>
                                                <div className="rt3_entry_head">
                                                    <p className="rt3_entry_title">{intern.job_title} — {intern.company_name}</p>
                                                    <p className="rt3_entry_date">
                                                        {intern.start_month} {intern.start_year} –{' '}
                                                        {(!intern.end_month || !intern.end_year) ? 'Present' : `${intern.end_month} ${intern.end_year}`}
                                                    </p>
                                                </div>
                                                <p className="rt3_entry_sub">{intern.location}{intern.employee_type ? ` • ${intern.employee_type}` : ''}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {educations?.length > 0 && (
                                    <div className="rt3_block">
                                        ...
                                        {educations.map((edu, i) => (
                                            <div className="rt3_entry" key={i}>
                                                <div className="rt3_entry_head">
                                                    <p className="rt3_entry_title">{edu.degree} in {edu.field_study}</p>
                                                    <p className="rt3_entry_date">
                                                        {(!edu.date || !edu.year) ? 'Still Studying' : `${edu.date} ${edu.year}`}
                                                    </p>
                                                </div>
                                                <p className="rt3_entry_sub">{edu.institute_name}{edu.location ? `, ${edu.location}` : ''}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {certificates?.length > 0 && (
                                    <div className="rt3_block">
                                        ...
                                        {certificates.map((cert, i) => (
                                            <div className="rt3_entry" key={i}>
                                                <div className="rt3_entry_head">
                                                    <p className="rt3_entry_title">
                                                        {cert.certificate_name}{cert.issuing_organization ? ` — ${cert.issuing_organization}` : ''}
                                                    </p>
                                                    {cert.issue_date && <p className="rt3_entry_date">{cert.issue_date}</p>}
                                                </div>
                                                {cert.description && <p className="rt3_entry_desc">{cert.description}</p>}
                                            </div>
                                        ))}
                                    </div>
                                )}
