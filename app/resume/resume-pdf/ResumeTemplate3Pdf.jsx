...
const ResumeTemplate3Pdf = ({ resume, palette = 'color-1', forceFallbackFont = false, fontFamily = 'Poppins' }) => {
  ...
  const SidebarHeading = ({ children }) => (
    <>
      <Text style={{ ...styles.sidebarHeading, color: theme.sidebarHeading }}>{children}</Text>
      <View style={{ ...styles.sidebarUnderline, backgroundColor: theme.sidebarUnderline }} />
    </>
  );

  const ContentHeading = ({ children }) => (
    <>
      <Text style={{ ...styles.contentHeading, color: accentColor }}>{children}</Text>
      <View style={{ ...styles.contentUnderline, backgroundColor: accentColor }} />
    </>
  );
      ...
      <View style={styles.entryRow}>
        <View style={styles.entryRail}>
          <View style={{ ...styles.ringIconOuter, borderColor: accentColor }}>
            <View style={{ ...styles.ringIconInner, backgroundColor: accentColor }} />
          </View>
          <View style={{ ...styles.entryRailLine, backgroundColor: accentColor }} />
        </View>
        <View style={styles.entryContent}>
          <View style={styles.entryTitleRow}>
            <Text style={{ ...styles.entryTitle, color: theme.entryTitle }}>{title}</Text>
            {dateLine ? <Text style={{ ...styles.entryDate, color: theme.entryDate }}>{dateLine}</Text> : null}
          </View>
          {subLine ? <Text style={{ ...styles.entrySubLine, color: theme.entrySubLine }}>{subLine}</Text> : null}
          {description ? <Text style={{ ...styles.entryDescription, color: theme.entryDescription }}>{description}</Text> : null}
        </View>
      </View>
    </View>
  );
        ...
        <View style={{ ...styles.header, backgroundColor: theme.headerBg }}>
          <View style={styles.headerPhotoWrap}>
            {profileSrc ? (
              ...
              <>
                <View style={{ ...styles.avatarHead, backgroundColor: 'rgba(255,255,255,0.25)' }} />
                <View style={{ ...styles.avatarBody, backgroundColor: 'rgba(255,255,255,0.25)' }} />
              </>
            )}
          </View>
          <View style={styles.headerInfo}>
            <Text style={styles.headerName}>{fullName}</Text>
            {jobLevel && <Text style={{ ...styles.headerTag, color: theme.headerTagText }}>{jobLevel}</Text>}
            {contactItems.length > 0 && (
              <View style={styles.headerContactRow}>
                {contactItems.map((item, idx) => (
                  <View key={idx} style={styles.headerContactItem}>
                    {item.type === 'location' ? <IconLocation color={theme.headerIconColor} /> : item.type === 'phone' ? <IconPhone color={theme.headerIconColor} /> : item.type === 'email' ? <IconEmail color={theme.headerIconColor} /> : <IconGlobe color={theme.headerIconColor} />}
                    <Text style={{ ...styles.headerContactText, color: theme.headerContactText }}>{item.label}</Text>
                  </View>
                ))}
              </View>
            )}
              ...
              <View style={styles.sidebarSection}>
                ...
                {skills.map((skill, idx) => (
                  <View key={idx} style={styles.skillBlock}>
                    <Text style={{ ...styles.skillName, color: theme.skillName }}>
                      {safeText(skill.skill_name)}
                      {skill.proficiency_level ? ` (${safeText(skill.proficiency_level)})` : ''}
                    </Text>
                    <View style={{ ...styles.skillBarTrack, backgroundColor: theme.skillBarTrack }}>
              ...
              <View style={styles.sidebarSection}>
                ...
                {languages.map((lang, idx) => (
                  <View key={idx} style={styles.bulletLine}>
                    <Text style={{ ...styles.bulletMark, color: theme.bulletMark }}>•</Text>
                    <Text style={{ ...styles.bulletText, color: theme.bulletText }}>
              ...
              <View style={styles.sidebarSection}>
                ...
                {socialItems.map((item, idx) => (
                  <View key={idx} style={styles.socialRow}>
                    <View style={{ width: 12, height: 12, alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>
                      {getSocialIcon(item.social_name, theme.socialIcon)}
                    </View>
                    <Text style={{ ...styles.socialText, color: theme.socialText }}>
            ...
            {hobbies.length > 0 && (
              <View style={styles.sidebarSection}>
                ...
                {hobbies.map((hobby, idx) => (
                  <View key={idx} style={styles.bulletLine}>
                    <Text style={{ ...styles.bulletMark, color: theme.bulletMark }}>•</Text>
                    <Text style={{ ...styles.bulletText, color: theme.bulletText }}>{safeText(hobby.hobbies || hobby)}</Text>
                  </View>
                ))}
              </View>
            )}
          ...
          <View style={styles.content}>
            {resume.summary && resume.summary.summary && (
              <View style={styles.contentSection}>
                ...
                <Text style={{ ...styles.bodyText, color: theme.bodyText }}>{safeText(resume.summary.summary)}</Text>
              </View>
            )}
            ...
            {resume.work_experiences && resume.work_experiences.length > 0 && (
              <View style={styles.contentSection}>
                ...
                {resume.work_experiences.map((work, idx) => (
                  <Entry
                    key={idx}
                    title={[safeText(work.job_title), work.company_name ? `— ${safeText(work.company_name)}` : ''].filter(Boolean).join(' ')}
                    dateLine={formatDateRange(work.start_month, work.start_year, work.end_month, work.end_year)}
                    subLine={[safeText(work.location), work.employment_type ? safeText(work.employment_type) : ''].filter(Boolean).join(' • ')}
            ...
            {resume.any_internships && resume.any_internships.length > 0 && (
              <View style={styles.contentSection}>
                ...
                {resume.any_internships.map((intern, idx) => (
                  <Entry
                    key={idx}
                    title={[safeText(intern.job_title), intern.company_name ? `— ${safeText(intern.company_name)}` : ''].filter(Boolean).join(' ')}
                    dateLine={formatDateRange(intern.start_month, intern.start_year, intern.end_month, intern.end_year)}
                    subLine={[safeText(intern.location), intern.employment_type ? safeText(intern.employment_type) : ''].filter(Boolean).join(' • ')}
                  />
                ))}
              </View>
            )}
            ...
            {resume.certificates && resume.certificates.length > 0 && (
              <View style={styles.contentSection}>
                ...
                {resume.certificates.map((cert, idx) => (
                  <Entry
                    key={idx}
                    title={[safeText(cert.certificate_name), cert.issuing_organization ? `— ${safeText(cert.issuing_organization)}` : ''].filter(Boolean).join(' ')}
