...
const buildbioDataText = (resume) => {
  const personal = resume.personal_infomation || {};
  const fullName = [personal.firstName, personal.lastName].filter(Boolean).join(' ') || resume.resume_name || 'Bio Data';
  const lines = [
    'BIO-DATA',
    '',
    fullName,
    personal.father_name ? `FATHER'S NAME: ${personal.father_name}` : '',
    personal.mother_name ? `MOTHER'S NAME: ${personal.mother_name}` : '',
    personal.date_of_birth ? `DATE OF BIRTH: ${personal.date_of_birth}` : '',
    personal.permanent_address ? `PERMANENT ADDRESS: ${personal.permanent_address}` : '',
    personal.present_address ? `PRESENT ADDRESS: ${personal.present_address}` : '',
    personal.caste ? `CASTE: ${personal.caste}` : '',
    personal.marital_status ? `MARITAL STATUS: ${personal.marital_status}` : '',
    personal.sex ? `SEX: ${personal.sex}` : '',
    personal.nationality ? `NATIONALITY: ${personal.nationality}` : '',
    personal.religion ? `RELIGION: ${personal.religion}` : '',
    personal.languages ? `LANGUAGES: ${personal.languages}` : '',
    '',
  ];

  if (resume.educations?.length) {
    lines.push('EDUCATIONAL QUALIFICATION');
    resume.educations.forEach((item) => lines.push(
      [item.exam_name || item.degree, item.board_university || item.institute_name, item.passing_year || item.year, item.marks].filter(Boolean).join(' | ')
    ));
    lines.push('');
  }
  if (resume.work_experiences?.length) {
    ...
    resume.work_experiences.forEach((item) => lines.push(
      `- ${[item.job_title, item.company_name, item.employee_type, item.location].filter(Boolean).join(' | ')}`,
      [item.start_month, item.start_year, item.end_month, item.end_year].filter(Boolean).join(' '),
      item.description || ''
    ));
    lines.push('');
  }
  if (resume.hobbies?.length) {
    ...
    resume.hobbies.forEach((item) => lines.push(`- ${item.hobbies || item}`));
    lines.push('');
  }
  lines.push('DECLARATION', 'I hereby declare that, the above information is true and correct to best of my knowledge.');

  return lines.filter((line, index, arr) => line || arr[index - 1]).join('\n');
};

const safeFileName = (resume) => (resume.resume_name || `${resume.personal_infomation?.firstName || 'bioData'}-${resume.id}`)
  .replace(/[^\w-]+/g, '-')
  .replace(/-+/g, '-')
  .replace(/^-|-$/g, '')
  .toLowerCase() || 'bioData';
...
export default function ResumeLists() {
  ...
  const downloadTXT = (resume) => {
    const blob = new Blob([buildbioDataText(resume)], { type: 'text/plain' });
  ...
  const downloadPDF = async (resume) => {
    try {
      ...
      if (!element) {
        toast.error('Bio-data preview not ready. Please try again.', { position: 'top-right', autoClose: 3000, theme: 'light' });
        return;
      }
      ...
      while (sourceY < trimmedCanvas.height) {
        ...
        const pageBreakY = remaining > pageCanvasHeight
          ? findPageBreakRow(trimmedCanvas, sourceY + sliceHeight, 160) - sourceY
          : sliceHeight;
  ...
  const downloadDOCX = (resume) => {
    const paragraphs = buildbioDataText(resume).split('\n').map((line, index) => {
  ...
  const BIO_DATA_CRITERIA = [
    { key: 'name', label: 'Name', points: 12, check: (r) => !!r.personal_infomation?.firstName },
    { key: 'father', label: "Father's Name", points: 10, check: (r) => !!r.personal_infomation?.father_name },
    { key: 'mother', label: "Mother's Name", points: 10, check: (r) => !!r.personal_infomation?.mother_name },
    { key: 'dob', label: 'Date of Birth', points: 10, check: (r) => !!r.personal_infomation?.date_of_birth },
    { key: 'permanent', label: 'Permanent Address', points: 14, check: (r) => !!r.personal_infomation?.permanent_address },
    { key: 'present', label: 'Present Address', points: 12, check: (r) => !!r.personal_infomation?.present_address },
    { key: 'personal', label: 'Caste / Status / Religion', points: 14, check: (r) => !!(r.personal_infomation?.caste && r.personal_infomation?.marital_status && r.personal_infomation?.religion) },
    { key: 'languages', label: 'Languages', points: 10, check: (r) => !!r.personal_infomation?.languages },
    { key: 'education', label: 'Education Table', points: 14, check: (r) => !!r.educations?.length },
    { key: 'hobbies', label: 'Hobbies & Interests', points: 4, check: (r) => !!r.hobbies?.length },
  ];

  const calcbioDataScore = (resume) =>
    BIO_DATA_CRITERIA.reduce((acc, c) => acc + (c.check(resume) ? c.points : 0), 0);

  const getbioDataSuggestions = (resume) =>
    BIO_DATA_CRITERIA.filter((c) => !c.check(resume));
    ...
    <>
      ...
      <section className="container-fluid custom-container small-hero-area">
        ...
        <div className="right-part d-none d-md-block">
          <img
            src="/front-assets/images/bio-data-hero.webp"
            className="img-fluid"
            width={500}
            height={360}
            alt="ResumeSathi bio-data maker dashboard"
            loading="eager"
            fetchPriority="high"
          />
        </div>
          ...
          {submittedResumeLists.length > 0 ? (
            <div className="row g-4">
              {submittedResumeLists.map((resume) => {
                const initials = (resume.personal_infomation?.firstName?.[0] || resume.resume_name?.[0] || 'B').toUpperCase();
                ...
                const bioDataScore = calcbioDataScore(resume);
                const bioDataCls = scoreClass(bioDataScore);
                return (
                  <div className="col-12 col-md-6 col-xl-4" key={resume.id}>
                    <div className="rl-card h-100">
                      ...
                      {/* Bio-data completion badge */}
                      {(() => {
                        const suggestions = getbioDataSuggestions(resume);
                        const radius = 22;
                        const circ = 2 * Math.PI * radius;
                        const dash = (bioDataScore / 100) * circ;
                        const arcColor = bioDataCls === 'ats-high' ? '#16a34a' : bioDataCls === 'ats-mid' ? '#d97706' : '#cc0000';
                        return (
                          <div className="rl-card__ats-wrap">
                            <div className={`rl-card__ats-badge ${bioDataCls}`}>
                              ...
                              <div className="rl-card__ats-inner">
                                <span className="rl-card__ats-score">{bioDataScore}%</span>
                                <span className="rl-card__ats-label">BIO</span>
                              </div>
                            </div>
                            {suggestions.length > 0 && (
                              <div className="rl-card__ats-tooltip">
                                <p className="rl-ats-tt-title">Complete your bio-data</p>
                      ...
                      <div className="rl-card__head">
                        ...
                        <div className="rl-card__title-group">
                          <p className="rl-card__name">{resume?.resume_name || 'Untitled Bio-Data'}</p>
                      ...
                      <div className="rl-card__meta">
                        ...
                        <div className="rl-card__meta-row">
                          ...
                          <span>{resume.personal_infomation?.father_name || 'Father name not added'}</span>
                        </div>
                        <div className="rl-card__meta-row">
                          ...
                          <span>{resume.personal_infomation?.date_of_birth || 'Date of birth not added'}</span>
                        </div>
                        <div className="rl-card__meta-row">
                          ...
                          <span>{resume.personal_infomation?.permanent_address || resume.personal_infomation?.present_address || 'Address not added'}</span>
                        </div>
                      </div>
                      ...
                      <div className="rl-card__actions">
                        <Link prefetch={false} href={`/bio-data/preview?id=${resume.id}`} className="rl-card__action-btn rl-card__action-btn--view">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z" /><circle cx="12" cy="12" r="3" /></svg>
                          Preview
                        </Link>
                        <Link prefetch={false} href={`/bio-data/select-theme/?id=${resume.id}`} className="rl-card__action-btn rl-card__action-btn--edit">
            ...
            <div className="row">
              <div className="col-12">
                <div className="rl-empty">
                  ...
                  <h2 className="rl-empty__title">No Bio-Data Yet</h2>
                  <p className="rl-empty__sub">You haven't created any bio-data yet. Start building a clean printable bio-data in minutes.</p>
                  <Link prefetch={false} href="/bio-data/resume-type" className="rl-create-btn">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M12 5v14M5 12h14" />
                    </svg>
                    Create Your First Bio-Data
                  </Link>
                </div>
              </div>
            </div>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-body py-4">
              ...
              <h5 className="heading">Delete This Bio-Data?</h5>
              <p className="sub-heading">Are you sure you want to delete this bio-data? This action cannot be undone.</p>
