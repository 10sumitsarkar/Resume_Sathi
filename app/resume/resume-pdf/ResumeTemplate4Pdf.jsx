...
const ResumeTemplate4Pdf = ({ resume, palette = 'color-1', forceFallbackFont = false, fontFamily = 'Poppins' }) => {
  ...
  const languagesLine = languages
    .map((lang) => `${safeText(lang.language)}: ${safeText(lang.proficiency_level)}`)
    .filter(Boolean)
    .join(', ');
  const hobbiesLine = hobbies.map((h) => safeText(h.hobbies || h)).filter(Boolean).join(', ');
  const socialLine = socialItems.map((s) => safeText(s.social_url)).filter(Boolean).join(', ');

  const SectionTitle = ({ children, topDivider = true }) => (
    <>
      {topDivider && <View style={{ ...styles.sectionDivider, backgroundColor: accentColor }}/>}
                ...
                <View key={idx} style={styles.entryBlock}>
                  <View style={styles.entryRow}>
                    <Text style={styles.entryTitle}>
                      {[safeText(work.job_title), safeText(work.company_name)].filter(Boolean).join(', ')}
                    </Text>
                    <Text style={styles.entryDate}>
                      {formatDateRange(work.start_month, work.start_year, work.end_month, work.end_year)}
                    </Text>
                  </View>
                  {work.location && <Text style={styles.entrySubLine}>{safeText(work.location)}</Text>}
                  {work.description &&
                    safeText(work.description)
                      .split(/\r?\n/)
                      .filter((line) => line.trim().length > 0)
                      .map((line, lIdx) => (
                        <Text key={lIdx} style={styles.entryDescription}>{line.replace(/^[-•]\s*/, '')}</Text>
                      ))}
                </View>
              ))}
            </View>
            ...
            <View style={styles.section}>
              ...
              {resume.any_internships.map((intern, idx) => (
                <View key={idx} style={styles.entryBlock}>
                  <View style={styles.entryRow}>
                    <Text style={styles.entryTitle}>
                      {[safeText(intern.job_title), safeText(intern.company_name)].filter(Boolean).join(', ')}
                    </Text>
                    <Text style={styles.entryDate}>
                      {formatDateRange(intern.start_month, intern.start_year, intern.end_month, intern.end_year)}
                    </Text>
                  </View>
                  {intern.location && <Text style={styles.entrySubLine}>{safeText(intern.location)}</Text>}
                </View>
              ))}
            </View>
          )}
              ...
              {resume.certificates.map((cert, idx) => (
                <View key={idx} style={styles.entryBlock}>
                  <View style={styles.entryRow}>
                    ...
                    <Text style={styles.entryDate}>{safeText(cert.issue_date)}</Text>
                  </View>
                  {cert.issuing_organization && (
                    <Text style={styles.entrySubLine}>{safeText(cert.issuing_organization)}</Text>
                  )}
                  {cert.description && (
                    <Text style={styles.entryDescriptionItalic}>{safeText(cert.description)}</Text>
                  )}
                </View>
              ))}
            </View>
                    ...
                    {socialItems.map((item, idx) => (
                      <View key={idx} style={styles.socialItemRow}>
                        ...
                        <Text style={styles.socialItemText}>
                          <Text style={styles.socialItemLabel}>{safeText(item.social_name)}: </Text>
