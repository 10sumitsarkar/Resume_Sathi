...
import { Document, Page, View, Text, Image, StyleSheet, Svg, Defs, LinearGradient, Stop, Rect, Link } from '@react-pdf/renderer';
...
const ResumeTemplate5Pdf = ({ resume, palette = 'color-1', forceFallbackFont = false, fontFamily = 'Poppins' }) => {
    ...
    <Document>
      <Page size="A4" style={pageStyle}>
        ...
        <View style={styles.bgLayer} fixed>
          <Svg width="100%" height="100%" viewBox="0 0 595 842" preserveAspectRatio="none">
            <Defs>
            ...
            <Rect x="0" y="0" width="595" height="842" fill="url(#pageGradient)" />
          </Svg>
        </View>
        ...
        <View style={styles.headerRow}>
          <View style={{ ...styles.photoBox, borderColor: accentColor }}>
            {profileSrc ? (
              ...
              <Text style={{ ...styles.photoFallbackText, color: accentColor }}>{initials || 'NA'}</Text>
            )}
          </View>
          <View style={styles.headerTextWrap}>
            <Text style={{ ...styles.nameText, color: accentColor }}>{fullName.toUpperCase()}</Text>
            {jobLevel ? <Text style={styles.roleText}>{jobLevel}</Text> : null}
          </View>
        </View>

        {/* ---------- CONTACT / ABOUT ---------- */}
        {(contactItems.length > 0 || (resume.summary && resume.summary.summary)) && (
            ...
            {workExperiences.map((work, idx) => (
              <View key={idx} style={styles.row}>
                <View style={styles.rowLabelCol}>
                  ...
                  <Text style={styles.entryOrg}>{safeText(work.company_name)}</Text>
                </View>
                <View style={styles.rowContentCol}>
                  <Text style={styles.entryTitle}>{safeText(work.job_title)}</Text>
                  {(work.location || work.employee_type) ? (
                    <Text style={styles.entrySub}>
                      {[safeText(work.location), safeText(work.employee_type)].filter(Boolean).join(' • ')}
                    </Text>
                  ) : null}
                  {work.description ? <Text style={styles.entryDesc}>{safeText(work.description)}</Text> : null}
                </View>
              </View>
            ))}
          </View>
            ...
            {internships.map((intern, idx) => (
              <View key={idx} style={styles.row}>
                <View style={styles.rowLabelCol}>
                  ...
                  <Text style={styles.entryOrg}>{safeText(intern.company_name)}</Text>
                </View>
                <View style={styles.rowContentCol}>
                  <Text style={styles.entryTitle}>{safeText(intern.job_title)}</Text>
                  {(intern.location || intern.employee_type) ? (
                    <Text style={styles.entrySub}>
                      {[safeText(intern.location), safeText(intern.employee_type)].filter(Boolean).join(' • ')}
                    </Text>
                  ) : null}
                </View>
              </View>
          ...
          <View style={styles.block}>
            ...
            {educations.map((edu, idx) => (
              <View key={idx} style={styles.row}>
                <View style={styles.rowLabelCol}>
                  <Text style={{ ...styles.entryDate, color: accentColor }}>
                    {!edu.date || !edu.year ? 'Still Studying' : `${safeText(edu.date)} ${safeText(edu.year)}`}
                  </Text>
                  <Text style={styles.entryOrg}>{safeText(edu.institute_name)}</Text>
                </View>
                <View style={styles.rowContentCol}>
                  <Text style={styles.entryTitle}>
                    {safeText(edu.degree)}{edu.field_study ? ` - ${safeText(edu.field_study)}` : ''}
                  </Text>
                  {edu.location ? <Text style={styles.entryDesc}>{safeText(edu.location)}</Text> : null}
                </View>
              </View>
            ))}
          </View>
        ...
        {certificates.length > 0 && (
          <View style={styles.block}>
            ...
            {certificates.map((cert, idx) => (
              <View key={idx} style={styles.row}>
                <View style={styles.rowLabelCol}>
                  {cert.issue_date ? <Text style={{ ...styles.entryDate, color: accentColor }}>{safeText(cert.issue_date)}</Text> : null}
                  {cert.issuing_organization ? <Text style={styles.entryOrg}>{safeText(cert.issuing_organization)}</Text> : null}
                </View>
                <View style={styles.rowContentCol}>
                  <Text style={styles.entryTitle}>{safeText(cert.certificate_name)}</Text>
                  {cert.description ? <Text style={styles.entryDesc}>{safeText(cert.description)}</Text> : null}
                </View>
              </View>
            ))}
          </View>
            ...
            <View style={styles.bulletGrid}>
              {languages.map((lang, idx) => (
                <View key={idx} style={styles.bulletItem}>
                  <View style={{ ...styles.bulletDot, backgroundColor: accentColor }} />
                  <Text style={styles.bulletText}>
                    {safeText(lang.language)}{lang.proficiency_level ? ` — ${safeText(lang.proficiency_level)}` : ''}
                  </Text>
                </View>
              ))}
            </View>
            ...
            {socialItems.map((social, idx) => {
              ...
              return (
                <View key={idx} style={styles.socialItem}>
                  <View style={styles.socialIconWrap}>{getSocialIcon(social.social_name, accentColor)}</View>
                  <Text style={styles.socialText}>{safeText(social.social_name)} - </Text>
                  <Link src={url} style={styles.socialLink}>{url}</Link>
                </View>
              );
            })}
          </View>
