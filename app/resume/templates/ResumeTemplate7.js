...
import "../resume-css/resumeTemplate7.css";
...
export default function ResumeTemplate7({ additionalClass, isStatic = false, resumeId, isForDownload = false }) {
  ...
  const ContactRow = ({ email, phone, address, website }) => {
    ...
    return (
      <div className="rt7_contact_row">
        {items.map((item, i) => (
          <span className="rt7_contact_item" key={i}>
            <span className="rt7_icon">{item.icon}</span>
  ...
  /* ─── Ledger-style entry helper (title/company left, date right) ─── */
  const Entry = ({ title, company, location, dateLabel, description, bullets }) => (
    <div className="rt7_entry">
      <div className="rt7_entry_top">
        <div className="rt7_entry_heading">
          <p className="rt7_entry_title">{title}</p>
          {company && <p className="rt7_entry_company">{company}</p>}
        </div>
        {dateLabel && <p className="rt7_entry_date">{dateLabel}</p>}
      </div>
      {location && <p className="rt7_entry_location">{location}</p>}
      {description && <p className="rt7_entry_desc">{description}</p>}
      {bullets && bullets.length > 0 && (
        <ul className="rt7_bullets">
          {bullets.map((line, j) => (
  ...
  return (
    <div
      ...
      data-selected-resume="ResumeTemplate7"
      ...
      >
        {showStaticData ? (
          <div className={`resume ResumeTemplate7 ${additionalClass}`}>
            ...
            {/* SKILLS */}
            <div className="rt7_section">
              <h3 className="rt7_section_title">Skills</h3>
              <div className="rt7_section_rule" />
              <div className="rt7_skills_grid">
                <ul>
                  {[
                    ["Cash register operation", "Intermediate"],
                    ["POS system operation", "Advanced"],
                    ["Sales expertise", "Expert"],
                    ["Teamwork", "Master"],
                  ].map(([n, l]) => (
                    <li key={n}>
                      <span className="rt7_skill_name">{n}</span>
                      <span className="rt7_skill_level">{l}</span>
                    </li>
                  ))}
                </ul>
                <ul>
                  {[
                    ...
                    ["Accurate money handling", "Advanced"],
            ...
            {/* EXPERIENCE */}
            <div className="rt7_section">
              <h3 className="rt7_section_title">Experience</h3>
              <div className="rt7_section_rule" />
              <Entry
                title="Retail Sales Associate"
                company="ZARA"
                location="New Delhi, India"
                dateLabel="Feb 2017 – Present"
                bullets={[
            ...
            {/* EDUCATION */}
            <div className="rt7_section">
              <h3 className="rt7_section_title">Education and Training</h3>
              <div className="rt7_section_rule" />
              <Entry
            ...
            {/* LANGUAGES */}
            <div className="rt7_section">
              <h3 className="rt7_section_title">Languages</h3>
              <div className="rt7_section_rule" />
              <div className="rt7_lang_grid">
                <div className="rt7_lang_item">
                  <span className="rt7_lang_name">Hindi</span>
                  <span className="rt7_lang_level">Native speaker</span>
                </div>
                <div className="rt7_lang_item">
                  <span className="rt7_lang_name">English</span>
                  <span className="rt7_lang_level">Fluent</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`resume ResumeTemplate7 ${additionalClass}`}>
            {/* HEADER */}
            <div className="rt7_header">
              <h1 className="rt7_name resume-name">
            ...
            {summary?.summary && (
              <div className="rt7_section">
                <h3 className="rt7_section_title">Summary</h3>
                <div className="rt7_profile">
                  <p className="rt7_text rt7_text_italic resume-about-me">{summary.summary}</p>
                </div>
              </div>
            )}
                ...
                <div className="rt7_skills_grid">
                  <ul>
                    {skills.slice(0, Math.ceil(skills.length / 2)).map((skill, i) => (
                      <li key={i}>
                        <span className="rt7_skill_name">{skill.skill_name}</span>
                        {skill.proficiency_level && (
                          <span className="rt7_skill_level">{skill.proficiency_level}</span>
                        )}
                      </li>
                    ))}
                  </ul>
              ...
              <div className="rt7_section">
                ...
                {work_experiences.map((work, i) => (
                  <Entry
                    ...
                    company={[work.company_name, work.employee_type].filter(Boolean).join(" · ")}
                    location={work.location}
                    dateLabel={`${`${work.start_month || ""} ${work.start_year || ""}`.trim()} – ${
            ...
            {any_internships?.length > 0 && (
              <div className="rt7_section">
                <h3 className="rt7_section_title">Internships</h3>
                <div className="rt7_section_rule" />
                {any_internships.map((intern, i) => (
                  <Entry
                    key={i}
                    title={intern.job_title}
                    company={[intern.company_name, intern.employee_type].filter(Boolean).join(" · ")}
                    location={intern.location}
                    dateLabel={`${`${intern.start_month || ""} ${intern.start_year || ""}`.trim()} – ${
              ...
              <div className="rt7_section">
                ...
                {educations.map((edu, i) => (
                  <Entry
                    ...
                    company={edu.institute_name}
                    location={edu.location}
                    dateLabel={!edu.date || !edu.year ? "Still Studying" : `${edu.date} ${edu.year}`}
                  />
                ))}
              </div>
            )}
            ...
            {certificates?.length > 0 && (
              <div className="rt7_section">
                <h3 className="rt7_section_title">Certificates</h3>
                <div className="rt7_section_rule" />
                {certificates.map((cert, i) => (
                  <Entry
                    ...
                    dateLabel={cert.issue_date ? formatDisplayDate(cert.issue_date) : null}
              ...
              <div className="rt7_section">
                ...
                <div className="rt7_lang_grid">
                  {languages.map((lang, i) => (
                    <div className="rt7_lang_item" key={i}>
                      <span className="rt7_lang_name">{lang.language}</span>
                      <span className="rt7_lang_level">{lang.proficiency_level}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {hobbies?.length > 0 && (
              <div className="rt7_section">
                <h3 className="rt7_section_title">Activities & Interests</h3>
                <div className="rt7_section_rule" />
                <ul className="rt7_hobbies_list">
                  {hobbies.map((hobby, i) => (
                    <li key={i}>{hobby.hobbies}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {social_medias?.length > 0 && (
              <div className="rt7_section">
                <h3 className="rt7_section_title">Social</h3>
                <div className="rt7_section_rule" />
                <ul className="rt7_social_list">
                  {social_medias.map((social, i) => {
                    ...
                    return (
                      <li key={i}>
                        <span className="rt7_social_icon">{getSocialIcon(social.social_name)}</span>
...
ResumeTemplate7.layoutStyle = "single-column";
