...
export default function ArticleDetailPageClient({ article: initialArticle, slug: initialSlug }) {
  ...
  const handleSuggestionSelect = (item) => {
    ...
    setShowSuggestions(false);
    closeOffcanvas();
    router.push(withTrailingSlash(`/jobs/${getSlug(item)}`));
  };
      ...
      <section className="container-fluid custom-container small-hero-area rk-article-hero">
        <div className="left-part">
          <div>
            ...
            <h1 className="fs-mob-20">{getTitle(article)}</h1>
          </div>
          <div className="rk-article-meta d-none">
            <span>
          ...
          {(begin || lastDate) ? (
  <div className="job-dates mt-2">
  {begin && (
    <div className="job-date-item">
      <span className="job-date-label">Apply From</span>
      <span className="job-date-value">
        {formatApplicationDate(begin)}
      </span>
    </div>
  )}

  {begin && lastDate && <div className="job-date-divider"></div>}

  {lastDate && (
    <div className="job-date-item">
      <span className="job-date-label">Last Date</span>
      <span className="job-date-value job-date-danger">
        {formatApplicationDate(lastDate)}
      </span>
    </div>
  )}
</div>
