...
import "../resume-css/resumeTemplate8.css";
...
export default function ResumeTemplate8({ additionalClass, isStatic = false, resumeId, isForDownload = false }) {
  ...
  /* ─── Timeline entry (dot + line marker, no text in the marker column) ─── */
  const TimelineEntry = ({ title, subLine, dateLabel, description, bullets, isLast }) => (
    <div className="rt8_tl_row">
      <div className="rt8_tl_rail">
        <span className="rt8_tl_dot" />
        {!isLast && <span className="rt8_tl_line" />}
      </div>
      <div className="rt8_tl_content">
        <div className="rt8_entry_top">
          <p className="rt8_entry_title">{title}</p>
          {dateLabel && <span className="rt8_date_pill">{dateLabel}</span>}
        </div>
        {subLine && <p className="rt8_entry_sub">{subLine}</p>}
        {description && <p className="rt8_body_text">{description}</p>}
        {bullets && bullets.length > 0 && (
          <ul className="rt8_bullets">
            {bullets.map((line, i) => (
  ...
  const TimelineSection = ({ title, items, mapFn }) => (
    <div className="rt8_section">
      <span className="rt8_section_tab">{title}</span>
      {items.map((item, i) => (
        <TimelineEntry key={i} {...mapFn(item)} isLast={i === items.length - 1} />
      ))}
    </div>
  );
  ...
  return (
    <div
      ...
      data-selected-resume="ResumeTemplate8"
    ...
    >
      ...
      >
        {showStaticData ? (
          <div className={`resume ResumeTemplate8 ${additionalClass}`}>
            <div className="rt8_banner">
              <p className="rt8_name resume-name">Rohan Kapoor</p>
              <p className="rt8_role resume-job-title">Full Stack Engineer</p>
              <div className="rt8_contact_row">
                <span className="rt8_contact_item"><span className="rt8_icon"><IconEmail /></span>rohan.kapoor@example.com</span>
                <span className="rt8_contact_item"><span className="rt8_icon"><IconPhone /></span>+91 98765 12345</span>
                <span className="rt8_contact_item"><span className="rt8_icon"><IconLocation /></span>Pune, India</span>
              </div>
            </div>
            <div className="rt8_banner_strip" />

            <div className="rt8_body">
              <div className="rt8_section">
                <span className="rt8_section_tab">Summary</span>
                <p className="rt8_body_text resume-about-me">
                  Full stack engineer with 5+ years of experience building scalable web applications
                  using React, Node.js, and cloud infrastructure. Passionate about clean architecture
                  and mentoring junior developers.
                </p>
              </div>

              <div className="rt8_section">
                <span className="rt8_section_tab">Skills</span>
                <div className="rt8_chips_row">
                  {["React", "Node.js", "TypeScript", "AWS", "PostgreSQL", "Docker", "GraphQL"].map((s) => (
                    <span className="rt8_chip" key={s}>{s}</span>
                  ))}
                </div>
              </div>

              <div className="rt8_section">
                <span className="rt8_section_tab">Work Experience</span>
                <TimelineEntry
                  title="Senior Software Engineer"
                  subLine="Vertex Systems — Pune, India"
                  dateLabel="Apr 2021 - Present"
                  bullets={["Led migration to microservices, reducing deployment time by 45%.", "Mentored a team of 4 junior engineers."]}
                />
                <TimelineEntry
                  title="Software Engineer"
                  subLine="Codeblend Labs — Mumbai, India"
                  dateLabel="Jun 2018 - Mar 2021"
                  bullets={["Built and shipped 3 major customer-facing features end-to-end."]}
                  isLast
                />
              </div>

              <div className="rt8_section">
                <span className="rt8_section_tab">Languages</span>
                {[["English", 5], ["Hindi", 5], ["Marathi", 3]].map(([lang, dots]) => (
                  <div className="rt8_lang_row" key={lang}>
                    <span className="rt8_lang_name">{lang}</span>
                    <span className="rt8_dots_row">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <span key={n} className={`rt8_dot ${n <= dots ? "filled" : ""}`} />
                      ))}
                    </span>
                  </div>
                ))}
          ...
          <div className={`resume ResumeTemplate8 ${additionalClass}`}>
            ...
            <div className="rt8_banner">
              ...
              <div className="rt8_contact_row">
                ...
                {personalInfomation?.website && (
                  <span className="rt8_contact_item resume-website"><span className="rt8_icon"><IconGlobe /></span>{personalInfomation.website}</span>
                )}
              </div>
            </div>
            <div className="rt8_banner_strip" />
            ...
            <div className="rt8_body">
              ...
              {summary?.summary && (
                <div className="rt8_section">
                  <span className="rt8_section_tab">Summary</span>
                  <p className="rt8_body_text resume-about-me">{summary.summary}</p>
                </div>
              )}
              ...
              {skills?.length > 0 && (
                <div className="rt8_section">
                  ...
                  <div className="rt8_chips_row">
                    {skills.map((skill, i) => (
                      <span className="rt8_chip" key={i}>{skill.skill_name}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {certificates?.length > 0 && (
                <TimelineSection
                  ...
                  mapFn={(cert) => ({
                    ...
                    dateLabel: formatDisplayDate(cert.issue_date),
              ...
              {languages?.length > 0 && (
                <div className="rt8_section">
                  ...
                  {languages.map((lang, i) => {
                    ...
                    return (
                      <div className="rt8_lang_row" key={i}>
                        <span className="rt8_lang_name">{lang.language}</span>
                        <span className="rt8_dots_row">
                          {[1, 2, 3, 4, 5].map((n) => (
                            <span key={n} className={`rt8_dot ${n <= dots ? "filled" : ""}`} />
                          ))}
              ...
              {hobbies?.length > 0 && (
                <div className="rt8_section">
                  <span className="rt8_section_tab">Hobbies</span>
                  <div className="rt8_chips_row">
                    {hobbies.map((hobby, i) => (
                      <span className="rt8_chip rt8_chip_neutral" key={i}>{hobby.hobbies || hobby}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {social_medias?.length > 0 && (
                <div className="rt8_section">
                  <span className="rt8_section_tab">Social Media</span>
                  <ul className="rt8_social_list">
                    {social_medias.map((social, i) => (
                      <li key={i}>
                        <span className="rt8_icon">{getSocialIcon(social.social_name)}</span>
...
ResumeTemplate8.layoutStyle = "single-column";
